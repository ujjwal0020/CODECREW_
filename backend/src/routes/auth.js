...
const { validateSignUpData } = require("../utils/validation");
const bcrypt = require("bcrypt");
const sendWelcomeEmail = require("../utils/sendEmail"); // Email sender function
...
authRouter.post("/signup", async (req, res) => {
  try {
    ...
    sendWelcomeEmail(emailId, firstName).catch((err) =>
...
authRouter.post("/login", async (req, res) => {
  ...
  try {
    ...
    // Set cookie securely (for Render HTTPS and cross-origin)
    res.cookie("token", token, {
      httpOnly: true,
      secure: true,
      sameSite: "None",
